...
>
> 如果你从 C/Java 来：`if (x > 0) { return true; }` → `if x > 0: return True`。û有括号，û有花括号，û有分号。你的手指会不自觉地打出 `;`，Ȼ后意识到 Python 不需要。
>
> 如果你从 JavaScript 来：`===` 不存在。Python 的 `==` 不会做类型转换。`[] == False` 是 `False`，不像 JS 那样让你怀疑人生。
>
> 无论你从哪里来，Python 都会在第一天教给你这个世界观：**代码是写给人看的，˳便让机器执行。**
...
- 变量是标ǩ，不是盒子：名称绑定 (Name Binding) 模型
- `is` vs `==`：身份比较 vs 值比较
...
>
> 在 C 中，`int x = 42;` 意味着在ջ上分配 4 字节，把 42 写进ȥ。`x` 是内存地址的别名。
> 在 Python 中，`x = 42` 意味着：在堆上创建一个 `int` 对象（值为 42），Ȼ后让名字 `x` 指向它。
> `x` 不是盒子，`x` 是贴在对象上的**标ǩ**。同一个对象可以有多个标ǩ（`y = x`），改名不影响对象。
> 这就是为什么 Python û有"传值"或"传引用"——它是**传对象引用 (Pass by Object Reference)**。
...
>
> Python 的 `dict` 基于哈希表实现，查找、插入、ɾ除均为 **平均 O(1)**。
> 但这个 O(1) 有两个前提：
> 1. **键必须可哈希**：`list` 不可哈希所以不能做键，`tuple` 可以（只要元素都可哈希）
> 2. **不退化**：极端情况下哈希冲突可导致 O(n)，但 CPython 的̽测策略让这几乎不会发生
>
> 从 Python 3.7 开始，`dict` **保证插入˳序**。这不是巧合——CPython 3.6 引入了紧凑字典 (Compact Dict) 实现，副作用就是保序，Ȼ后 3.7 把它变成了语言规范。

| 数据结构 | 查找 | 插入 | ɾ除 | 有序 | 可变 | ȥ重 | 适用场景 |
|---------|------|------|------|------|------|------|---------|
| `list` | O(n) | O(1)* | O(n) | ✅ | ✅ | ❌ | 有序集合、ջ |
| `tuple` | O(n) | — | — | ✅ | ❌ | ❌ | 不可变记录、字典键 |
| `dict` | O(1) | O(1) | O(1) | ✅* | ✅ | 键唯一 | 键值映射、缓存 |
| `set` | O(1) | O(1) | O(1) | ❌ | ✅ | ✅ | ȥ重、成员判断、集合运算 |
| `deque` | O(n) | O(1)两端 | O(1)两端 | ✅ | ✅ | ❌ | 队列、˫端操作 |

*注：list 的 append 是均̯ O(1)；dict 从 3.7+ 保持插入˳序*
...
> 🧰 **Toolbox：`pathlib` vs `os.path` — 新旧之ս**
>
> ```python
> # 旧方式 — os.path（字符串ƴ接，容易出错）
> import os
...
> 3. **鸭子类型**：你不检查类型，你尝试使用——如果它走·像鸭子、叫起来像鸭子，它就是鸭子
>
> 当Ȼ，最 Pythonic 的写法是：`value = dictionary.get(key, default)` 😄
...
> ```
>
> 核心原则：**直接表达你的意图**。如果你想遍历元素，就遍历元素，不要遍历索引再ȡ元素。

---

### [实ս项目：命令行任务管理器](./projects/cli-task-manager/)
...
3. **动手实践** — 修改示例代码，观察结果
4. **完成练习** — 独立完成章节练习
5. **对比 JS/TS** — 如果学过 JS/TS，˼考 Python 的不同之处
...
- [Real Python](https://realpython.com/) — 高质量 Python 教程网վ
